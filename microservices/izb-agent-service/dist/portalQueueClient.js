...
function getPortalConfig() {
    return {
        ...
        agentId: process.env.AGENT_ID?.trim() || 'izb-agent',
    };
}
async function claimNextPayment() {
    const { appApiUrl, bankPullApiKey, agentId } = getPortalConfig();
    if (!appApiUrl || !bankPullApiKey) {
        console.warn('[IZB] Portal claim skipped: APP_API_URL or BANK_PULL_API_KEY is missing');
        return null;
    }
    const response = await fetch(`${appApiUrl}/api/v1/agent_queue/claim`, {
        ...
        body: JSON.stringify({ bankCode: 'IZB', agentId }),
    });
    if (!response.ok) {
...
async function reportQueueStatus(queueId, report) {
    const { appApiUrl, bankPullApiKey, agentId } = getPortalConfig();
    if (!appApiUrl || !bankPullApiKey) {
        console.warn('[IZB] Portal reporting skipped: APP_API_URL or BANK_PULL_API_KEY is missing');
        return;
    }
