...
function normalizePortalItems(portalJson) {
    if (!portalJson)
        return [];
    if (Array.isArray(portalJson))
        return portalJson;
    if (typeof portalJson !== 'object')
        return [portalJson];
    const record = portalJson;
...
function mapItem(it) {
    const record = (typeof it === 'object' && it !== null ? it : {});
    const p = (typeof record.payment === 'object' && record.payment !== null ? record.payment : record);
    const get = (keys) => {
...
async function proxyPostCashbook(req, res) {
    try {
        const providedKey = String(req.headers['x-api-key'] || '');
        if (!AGENT_API_KEY || providedKey !== AGENT_API_KEY) {
