...
app.post('/payments', async (req, res) => {
  ...
  if (isIzbServicePayload(body)) {
    ...
    const job = await paymentQueue.add('send-izb-payment', { payment: body, queueId }, {
  ...
  if (bankCode !== 'IZB') {
    return res.status(400).json({ success: false, error: 'bankCode must be IZB' });
  }
  ...
  const job = await paymentQueue.add('send-izb-payment', { payment, queueId }, {
    attempts: Number(process.env.JOB_ATTEMPTS || 3),
    backoff: { type: 'exponential', delay: Number(process.env.JOB_BACKOFF_MS || 5000) },
    removeOnComplete: true,
    removeOnFail: false,
...
app.listen(port, () => {
  ...
  console.log('POST /payments with either { bankCode: "IZB", payment: {...} } or direct IZB payload { service, request }');
});
